...
def domain2RangesByIpInfo(domain):
    print("     - domain2Ranges by IPI")
    companyId = None
    global countIpInfoData

    if (domain != "" and domain is not None and mayWeQueryIpInfo()):
        try:
            ...
            if company != None and company != "":
                ...
                cnx = mysql.connector.connect(host="localhost", user="truffle", password="anothertry",
                                              database="truffle")
                cursor = cnx.cursor(buffered=True)
                cursor.execute(sql, values)
                cnx.commit()
...
def domain2linkedinByProxycurl(domain):
    result = None
    try:
        sql = "select universal_name_id from proxycurl JOIN (SELECT companyId from websites where website=%s) w on proxycurl.company_ID=w.companyId "
        values = (domain,)
        cnx = mysql.connector.connect(host="localhost", user="truffle", password="anothertry", database="truffle")
        cursor = cnx.cursor(buffered=True)
        cursor.execute(sql, values)
