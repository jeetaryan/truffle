...
#API keys
pcKey = '567d7f13-835b-463d-b85a-179dcef18bdf'
    ...
    if (ipAddress != "" and ipAddress != None):
        ...
        try:
            ...
            if "." in ipAddress:
                ...
                sql = "SELECT companyId FROM ip_ranges where ipStartv4 <=%s and ipEndv4 >= %s"
                values = (ipAddress_int, ipAddress_int)
            elif ":" in ipAddress:
                sql = "SELECT companyId FROM ip_ranges where ipStartv6 <=%s and ipEndv6 >= %s"
                values = (format(ipaddress.IPv6Address(ipAddress)), format(ipaddress.IPv6Address(ipAddress)))             
            else:
                return companyId
    ...
    if  mayWeQueryKickfire():
        try:
            ...
            if response['status'] == "success":
                ...
                companyId = getCompanyId(name, domain, 1)
...
def ip2companyByProxycurl(ipAddress):
    ...
    if mayWeQueryProxycurl():
        try:
            ...
            companyId = processProxyCurl(response, 0, None)
            if companyId:
    ...
    try:
        ...
        if (res):
            ...
            if (extra):
                ...
                hq_country = res.get("hq").get("country") if res.get("hq") else None
            ...
            if (name != None):
                ...
                if companyId==None: companyId = getCompanyId(name, domain, 1)
...
def domain2linkedinByProxycurl(domain):
    # todo: first try to look in our DB, introduce a second parameter (queryApisIfUnknown)
    result = None
    if mayWeQueryProxycurl():
...
def domain2linkedin(domain):
    result = None
    result = domain2linkedinByScrapingWebsite(domain)
    if result == None: result = domain2linkedinByProxycurl(domain)
    if result == None: result = domain2linkedinByKickfire(domain)
    return result
...
def linkedin2companyByProxycurl(linkedInProfile, companyId):
    ...
    if (linkedInProfile != "" and linkedInProfile != None and  mayWeQueryProxycurl()):
        try:
            ...
            success = processProxyCurl(response, 1, companyId)
            if success:
...
def getLatLon(street, city, postalcode, state, country):
    print("     - get LAT/LON")
    try:
        params = {'street': street, 'city': city, 'country': country, 'postalcode': postalcode, 'state': state, 'format': "json", 'limit': "1", 'addressdetails': "1"}
...
def insertAddress(companyId, line_1, line_2, postal_code, city, country, country_short, state, state_short, is_hq, lat, lon):
    ...
    if (lat == 0 or lon == 0 or lat == '' or lon =='' or lat == None or lon == None):
        ...
        latlon = getLatLon(line_1, city, zip, state, country)
        if latlon:
            lat = latlon[0]
            lon = latlon[1]
        else:
...
def insertWebsite(companyId, website, source):
    ...
    try:
        website2 = clnWebsite(website)
        if website2:
            values = (companyId, website2, source)
            sql = "INSERT ignore INTO websites (companyId, website, source) values(%s, %s, %s)"
            cursor.execute(sql, values)
            site_id = cursor.lastrowid
            cnx.commit()
            getScreenshot(site_id, website)
            return 1
        else:
            return 0
    except BaseException as e:
...
def updateIpRangesByIpi(domain, companyId, forceEvenIfYoungData):
    try:
        ...
        if not forceEvenIfYoungData:
            sql = "select lastModifiedOn from ip_ranges where companyId=%s LIMIT 1"
            values=(companyId, )
            cursor3.execute(sql, values)
            result = cursor3.fetchone()
            if result and (result[0] < ( datetime.now() - dt.timedelta(weeks=4))):
                oldData = True
...
def getCompanyId(name, domain, createNewIfNotFound):
    ...
    # it is calles by kickfire, proxcurl and ipinfo functions.
    companyId = None
    
    # todo query free kf API to translate name to domain if domain is null
    
    if domain: companyId=domain2companyId(domain, 0)
    ...
    if not companyId and createNewIfNotFound:
        if domain:
            companyId=domain2companyId(domain, 1)
            # do not enrich!
        
        else: companyId = generateNewCompanyId(name)
...
def companyName2companyIdbyKickfire(name):
    try:
        ...
        if res and len(res)>0:
            companyId = res[0][0]
            print("   - existing CompanyId ", companyId)
            return companyId
    except BaseException as e:
        print("Exception caught while generating new companyId: ", e)
        traceback.print_exc()
...
def domain2companyId(domain, createIfNotExist):
    companyId= None
    try:
        sql = "select companyId from websites where website=%s"
        values = (domain,)
        cursor.execute(sql, values)
        res = cursor.fetchall()
        if res and len(res)>0:
...
def getScreenshot(site_id, domain):
    options = Options()
    options.headless = True    
    #options.add_argument('--user-agent="Mozilla/5.0 (compatible; GoogleBot/2.1"')
    ...
    #if domain == None:
    #    domain = companyId2Domain(site_id)
    #if site_id == None:
    #    site_id = domain2companyId(domain, 1)
    ...
    try:
        ...
        if not button:
            try: button = driver.find_element("xpath", "//a[contains(text(), 'onsent')]")
            except BaseException as e: pass
        if not button:
        ...
        if screenShot:
            ...
            domain = "www." + domain
            return getScreenshot(site_id, domain)
        else:
    ...
    if string:
        ...
        if num and num != "":
            ...
            if phonenumbers.is_possible_number(my_number):
                return phonenumbers.format_number(my_number, phonenumbers.PhoneNumberFormat.INTERNATIONAL)
    return None

def clnEmail(string):
    return email_normalize.normalize(cln(string))
...
def translateData():
    ...
    for x in data:
        ...
        visitId = x[0]
        ipAddress_int = x[1]
        ipAddress = int2ip(ipAddress_int) 
        companyId = ip2company(ipAddress, 1)

        try:
            # update all records with same IP
            values = (companyId, 6, visitId)
            sql = "update visits set companyId=%s, status=%s where visitId=%s"
            cursor3.execute(sql, values)
            cnx3.commit()
            # update temptable translatedDataDone
            sql = "update temptable set translatedDataDone=%s where id=%s"
            value = (countTranslatedData,1)
            cursor3.execute(sql, value)
            cnx3.commit()
        except BaseException as e:
...
def updateCompanies():
    ...
    for x in data:
        ...
        if cursor3.rowcount>0:
            ...
            ip2companyByIpInfo(ipAddress, companyId)
        else:
            deleteCompanyAndVisits(companyId)
        

#getScreenshot(4090, "telefonica.de")
...
for row in rows:
    ...
    if website != row[1]:
        try:
            ...
            print("Domain seems to exist: ", website)
            try:
                ...
                print("Exception caught while updating visits after IP translation: {}".format(e))
                traceback.print_exc()
    getScreenshot(row[0], row[1])
    getLogo(row[2], row[1])
